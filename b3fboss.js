...
*/
  ...
  var bossEnergy = 100;               // ボスの体力（初期値は最大体力）
  ...
  const typingTextCharNumber = 3;   // typingTexts各要素の文字数
  ...
  */ 
  upstairButton.onclick = () => {
    document.location.href = "./b2f.html";
  }
  ...
  */
  document.onkeydown = () => {
    ...
    if (flgTypeEnd === 0) {      // flg=1の時（=タイピングを終えた時）は入力文字の正誤判断をしない
      ...
      {
        sampleDivided[typedCount].style.color = 'blue';
        points = points + 10;
      } else {    // 入力した文字が誤っているとき
        sampleDivided[typedCount].style.color = 'red';
        points = points - 10;
      }
      typedCount++;
      ...
      if (dispTypeText.length === typedCount) {
        ...
        if (bossEnergy === 0) {   // ボスの残り体力がゼロになった時
          ...
          statusText.innerText = 'お見事！ボスを倒しました！';
