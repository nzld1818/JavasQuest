...
*/
  ...
  var remainingWalkCount = 200;           // 残り歩数のカウント用
  ...
  // 定数定義部
  const allRemainingwalkCounts = 200;     // スタートからゴールまでの歩数を入れておく変数

  const typingTextCharNumber = 4;         // typingTextsの各要素の文字数
 ...
 function displayStatus() {
  ...
  statusText.innerText = '地下1階まであと ' + remainingWalkCount + ' 歩!';
...
　}
  ...
  */
  document.onkeydown = () => {
    const maxPoints = 40;                   // ノーミス時の点数（ボーナス点は含まない）
      ...
      if (dispTypeText.length === typedCount) {
        ...
        */
        ...
        if (remainingWalkCount !== 0) {    // 残り歩数が0でない時
          ...
          setTimeout(() => {
            removeAllChildren(resultDivided);
            resultText.innerText = '「おっ！上へ登るハシゴだ！」';
            resultDivided.appendChild(resultText);
          }, 1000)
          setTimeout(() => {
            displayBossGraphic();
            removeAllChildren(statusDivided);
            statusText.innerText = '「私の出題をお前に解けるかな？ フフフ・・・」';
            statusDivided.appendChild(statusText);
          }, 3000)
          setTimeout(() => {
            displayBossGraphic();
