...
*/
  ...
  var bossEnergy = 150;               // ボスの体力（初期値は最大体力）
  ...
  const typingTextCharNumber = 5;   // typingTexts各要素の文字数
    ...
    if (flgTypeEnd === 0) {      // flg=1の時（=タイピングを終えた時）は入力文字の正誤判断をしない
      ...
      if (dispTypeText.length === typedCount) {
        ...
        if (bossEnergy === 0) {   // ボスの残り体力がゼロになった時
          ...
          statusText.innerText = '「成長しましたね。講師として本当にうれしいです」';
          statusDivided.appendChild(statusText);
          setTimeout(() => {
            removeAllChildren(resultDivided);
            removeAllChildren(statusDivided);
            statusText.innerText = '「これからも頑張っていきましょう」';
            statusDivided.appendChild(statusText);  
          }, 2500);    
          setTimeout(() => {
            removeAllChildren(statusDivided);
            statusText.innerText = 'そう言い残すとドラゴンは去っていった。';
            statusDivided.appendChild(statusText);  
          }, 5000);           
          setTimeout(() => {
            ...
            remarksText.innerText = '地上へのハシゴが見える。登りますか？'; 
            remarksDivided.appendChild(remarksText);
          ...
          setTimeout(() => {
            remarksDivided.style.color = "#ffffff";
            remarksText.innerText = 'もう一度ラスボスと戦いますか？';
            remarksDivided.appendChild(remarksText);
