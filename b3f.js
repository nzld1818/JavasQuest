...
*/
  ...
  var remainingWalkCount = 100;           // 残り歩数のカウント用
  ...
  // 定数定義部
  const allRemainingwalkCounts = 100;     // スタートからゴールまでの歩数を入れておく変数

  const typingTextCharNumber = 3;         // typingTextsの各要素の文字数
 ...
 function displayStatus() {
  ...
  statusText.innerText = '地下2階まであと ' + remainingWalkCount + ' 歩!';
...
　}
  ...
  */ 
  bossButtleButton.onclick = () => {
    document.location.href = "./b3fboss.html";
  }
  ...
  */
  document.onkeydown = () => {
    ...
    const bonusPointSeconds = 2;            // ボーナスポイント対象となる秒数
    if (typedCount === 0) {
      ...
      if (dispTypeText.length === typedCount) {
        ...
        */
        ...
        if (remainingWalkCount !== 0) {    // 残り歩数が0でない時
          ...
          setTimeout(() => {
            removeAllChildren(resultDivided);
            resultText.innerText = '「あっ！上へ登るハシゴだ！」';
            resultDivided.appendChild(resultText);
          }, 1000)
          setTimeout(() => {
            displayBossGraphic();
            removeAllChildren(statusDivided);
            statusText.innerText = '「この先へは進ませんぞ！俺を倒してから行け！」';
            statusDivided.appendChild(statusText);
          }, 3000)
          setTimeout(() => {
            displayBossGraphic();
            removeAllChildren(remarksDivided);
            remarksText.innerText = 'なんとボスが現れた！戦いますか？';
